...
if(!consent_column %in% names(clean_dataset)){stop(paste0(consent_column, " not found in the clean data"))}
...
if(!sampling_frame_strata_column %in% names(sample_frame)){stop(paste0(sampling_frame_strata_column, " not found in the sample frame"))}
if(!sampling_frame_target_survey_column %in% names(sample_frame)){stop(paste0(sampling_frame_target_survey_column, " not found in the sample frame"))}
if(!clean_dataset_strata_column %in% names(clean_dataset)){stop(paste0(clean_dataset_strata_column, " not found in the clean data"))}
...
actual_df <- clean_dataset |> dplyr::group_by(!!rlang::sym(clean_dataset_strata_column)) |> dplyr::summarise(
    Collected = dplyr::n()
  )
