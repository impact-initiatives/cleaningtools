...
#'
#'
#'check_cleaning_log(raw_dataset = test_data,raw_data_uuid_column = "uuid",cleaning_log =cleaning_log_test,
...
#'
#'create_clean_data(raw_dataset = test_data,raw_data_uuid_column = "uuid",cleaning_log = cleaning_log_test,
#'                cleaning_log_change_type_column =  "change_type",
#'                change_response_value = "change_res",
...
create_clean_data <- function(raw_dataset,
  ...
  cleaning_log <- cleaning_log |> dplyr::mutate(
    change_type_created_f = case_when(!!rlang::sym(cleaning_log_change_type_column) %in% change_response_value ~ "change_response",
  ...
  check_not_entry_value_in_change_type <- unique(cleaning_log[[cleaning_log_change_type_column]])[!unique(cleaning_log[[cleaning_log_change_type_column]]) %in% all_type]
  ...
  cleaning_log[[cleaning_log_question_column]]<-cleaning_log[[cleaning_log_question_column]] %>% trimws()
  cleaning_log[[cleaning_log_new_value_column]]<-cleaning_log[[cleaning_log_new_value_column]] %>% trimws()
  cl_change_type_options<- c("change_response",  "remove_survey", "blank_response","no_action")
  cl_change_response<- cleaning_log %>% dplyr::filter(change_type_created_f %in% c( cl_change_type_options[1],cl_change_type_options[3]))
  cl_change_response<- cl_change_response %>%
    dplyr::mutate(
      !!cleaning_log_new_value_column:=ifelse(change_type_created_f==cl_change_type_options[3],NA,!!rlang::sym(cleaning_log_new_value_column))
    )
  cl_remove_survey<- cleaning_log %>% dplyr::filter(change_type_created_f == cl_change_type_options[2])

  if(all(cl_change_response[[cleaning_log_question_column]] %in% colnames(raw_dataset))==F){
    problem_question_in_cl<-cl_change_response[[cleaning_log_question_column]][cl_change_response[[cleaning_log_question_column]] %in% colnames(raw_dataset)==FALSE]
  ...
  if(all(cleaning_log[[cleaning_log_uuid_column]] %in% raw_dataset[[raw_data_uuid_column]] ==F)){
    problem_uuid_in_cl<-cleaning_log[[cleaning_log_uuid_column]][cleaning_log[[cleaning_log_uuid_column]] %in% c(raw_dataset[[raw_data_uuid_column]],"all_data")==FALSE]
  ...
  if(nrow(cl_change_response)>0){
    for(i in 1:nrow(cl_change_response)){
      ...
      cl_uuid_temp<-cl_change_response[[cleaning_log_uuid_column]][i]
      cl_question_temp<-cl_change_response[[cleaning_log_question_column]][i]
      cl_new_val_temp<-cl_change_response[[cleaning_log_new_value_column]][i]

      if(cl_uuid_temp!="all_data"){
        raw_dataset[raw_dataset[[raw_data_uuid_column]]==cl_uuid_temp,cl_question_temp]<-cl_new_val_temp}
  ...
  if(nrow(cl_remove_survey)>0){
    raw_dataset<- raw_dataset %>% dplyr::filter(!!rlang::sym(raw_data_uuid_column) %in% cl_remove_survey[[cleaning_log_uuid_column]]==FALSE)
