...
check_outliers <- function(dataset,
  ...
  if(!is.list(dataset)){stop("Input must be a dataframe or list.")}
  ...
  dataset <- type.convert(dataset, as.is = TRUE,na.string= c(""," ")) |> dplyr::rename(
  ...
  if(remove_choice_multiple == T ){columns_not_to_check <- c(columns_not_to_check,all_select_multiple_cols)}
  ...
  cols_name_exist_in_loop_numeric <- dataset %>% dplyr::select_if(is.numeric) %>% dplyr::select(-starts_with("X"))%>% names()
  cols_name_exist_in_loop_int <- dataset %>% dplyr::select_if(is.integer) %>% dplyr::select(-starts_with("X"))%>% names()
  ...
  for (x in cols_name_exist_in_loop) {
    ...
    outlier_checks[[x]]  <-  dataset %>% dplyr::mutate(
    ...
    outlier_checks[[paste0("log_",x)]] <-  dataset %>% dplyr::mutate(
