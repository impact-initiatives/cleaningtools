...
create_formated_wb <- function(write_list,
  ...
  headerStyle <- openxlsx::createStyle(fontSize = header_front_size,
  ...
  bodyStyle <- openxlsx::createStyle(fontSize = body_front_size,
                                     fontName = body_front,
                                     border = "TopBottomLeftRight ",
                                     borderColour = "#4F81BD",
                                     valign = "center",
  ...
  for(i in 1:number_of_sheet ){
    ...
    if(!is.null(cols_for_color)){
      ...
      for(x in u){
        ...
        style <- openxlsx::createStyle(fgFill=random.color,
...
create_xlsx_cleaning_log <- function(write_list,
  ...
  if("validation_rules" %in% names(write_list)){stop(glue::glue("The list currently has an element named `validation_rules`. Please consider renaming it."))}
  ...
  if (!is.null(kobo_survey) & !is.null(kobo_choices) & use_dropdown == TRUE & exists("data.val",inherits = FALSE)) {
    ...
    for (r in 1:nrow(cl)){
      ...
      if (cl[r,"question"] %in% colnames(data.val) & as.character(cl[r,"uuid"]) != "all"){
        openxlsx::dataValidation(workbook, sheet = cleaning_log_name, cols = which(colnames(cl)=="new_value"),
                                 rows = r+1, type = "list",
    ...
    openxlsx::dataValidation(workbook, sheet = cleaning_log_name, cols  = col_number,
  ...
  if(is.null(output_path)){return(workbook)}
