...
test_that("outlier_check returns correct results", {
  ...
  one_value_outlier <- data.frame(uuid = paste0("uuid_", c(91:100)),
                                  issue = "outlier (normal distribution)",
                                  question = "one_value",
                                  old_value = df_outlier[91:100, "one_value"]

  )
  expense_outlier <- data.frame(uuid = c(paste0("uuid_", c(99:100)), paste0("uuid_", c(97:98))),
  ...
  income_outlier <- data.frame(uuid = paste0("uuid_", c(1,2,3)),
                               issue = "outlier (log distribution)",
                               question = "income",
                               old_value = c(60,0,80))


  yy_outlier <- data.frame(uuid = paste0("uuid_", 100),
                           issue = "outlier (normal distribution)",
