...
test_that("test that if the new names exists, it will stop", {
  test_data <- data.frame(
    uuid = c(1:3),
    percentage_missing = c(.8, .8, .9)
  ) %>% dplyr::rename(`_uuid` = uuid)
  testthat::expect_error(
    add_percentage_missing(test_data),
...
test_that("test that if the new names exists, it will stop", {
  test_data <- data.frame(
    uuid = c(1:3),
    perc_missing = c(.8, .8, .9)
  ) %>% dplyr::rename(`_uuid` = uuid)
  testthat::expect_error(
    add_percentage_missing(test_data, column_name = "perc_missing"),
...
test_that("test that the columns are added with the correct values with a kobo", {
  ...
  kobo_survey <- data.frame(
    type = c("uuid", "integer", "select_one choice2", "select_one choice3", "select_multiple choice4"),
  ...
  testthat::expect_equal(
    add_percentage_missing(data_test,
  ...
  testthat::expect_error(add_percentage_missing(data_test,
    kobo_survey = kobo_survey,
...
test_that("it returns the dataframe with only 1 added column", {
  data_test <- data.frame(
    ...
    dplyr::rename(`_uuid` = uuid)
  expect_equal(
    ncol(add_percentage_missing(data_test)),
    ncol(data_test) + 1
  )
})
...
test_that("check_percentage_missing returns the correct values", {
  ...
  no_flag_expected_output <- list(
    checked_dataset = no_flag_data_test,
  ...
  renamed_expected_output <- list(
    checked_dataset = renamed_data_test,
    percentage_missing_log = dplyr::tibble(
      uuid = "z",
      issue = "Percentages of missing values from this survey is different from others",
      question = "blanks",
  ...
  renamed_log_expected_output <- list(
    checked_dataset = data_test,
